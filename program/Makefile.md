...
### 基本的make指令
* `make -B` 不考虑时间戳，所有的文件都重新编译，功能相当于`make clean && make`。
* `make clean` 在Makefile文件中有具体实现命令，基本上是删除掉所有中间文件和最终目标文件。
* `make -j4` 设置多线程（4个线程）编译，在源文件比较多的时候非常受用。

### 进阶注意事项：
* 在修改了头文件之后，make指令不能良好的执行其功能，需要重新编译（`make -B`)。
