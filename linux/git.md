...
#### 基本操作：pull
`git pull` 命令是将远程的修改更新到本地，要求本地相对于远程没有修改（如果有，则需要进行merge操作）。如果远程修改的文件和本地修改的文件不是同一个文件，可以直接merge。如果远程修改的文件和本地修改的文件有冲突，那么尽量选择手动merge，自动merge会出现部分代码出现两遍，而且带有相关的修改信息，整体代码显示非常乱。
